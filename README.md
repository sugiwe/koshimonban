...
作者が座りっぱなしのデスクワークで腰を痛めたので作った。
30分に1回立ち上がるのが理想だが、意志では続かなかった。

**このアプリの本質は機能の豊かさではなく「無視されにくさ」にある。**
設計上の判断はすべてその優先順位で決めている。詳しくは [SPEC.md](SPEC.md)。
...
- 設定した作業時間帯の中で、決まった間隔（既定 30分）ごとに発動する
- 発動すると**全ディスプレイが全画面で覆われる**。他アプリのネイティブフルスクリーンの上にも出る
- 3・2・1 のあと、登録しておいたストレッチ動画（YouTube またはローカルファイル）が再生される
- 既定 3分間。「腰を守った✌️」で切り上げられる
- 「腰より仕事💀」でスキップできるが、**最初の5秒間は押せず**、押すと理由を1つ選ばされる
- `Esc` では閉じない（意図的にそうしてある）
- 完了・短縮・スキップ・取りこぼしが記録され、達成率とヒートマップで見返せる

## 逃げ道

画面を奪うアプリなので、先に抜け方を書いておく。
...
- macOS 14 以上
- ビルドに Xcode 16 以上（開発は Xcode 26 / macOS 26 で行っている）

外部ライブラリへの依存はない。標準フレームワークのみ。

## 使ってみる

**配布用のビルド済みアプリは用意していない。** 自分でビルドする必要がある。
...
`deploy.sh` がビルド → `/Applications/Koshimonban.app` へ配置 → 起動までを行う。
Dock にはアイコンが出ない（`LSUIElement`）。メニューバー右上の人型アイコンから操作する。

Xcode を入れた直後は、先に次が必要になることがある。
...
より詳しい手順は [docs/SETUP.md](docs/SETUP.md) にある。

### 最初にやる設定

メニューバーのアイコン →「設定を開く」から。

1. **「作業時間帯」タブ** — 実際の勤務時間に合わせる（既定は月〜金の 10:00–12:00 と 14:00–17:00）。
   下部の「今日の発動予定」に実際の発動時刻が並ぶので、意図どおりか確認する
2. **「動画」タブ** — ストレッチ動画を登録する。0件でも動く（テキストとカウントダウンだけになる）。
...
スリープや画面ロックで発動時刻を過ぎた場合は、休憩が時間帯に収まるうちは追いつく。
間に合わなければ「取りこぼし」として記録に残る。
...
`deploy.sh` が `/Applications` に置くのはこのため。パスが食い違っている場合は設定画面に警告が出る。
...
**このデータはマシンごとにローカル保存される。** 複数の Mac で使うと記録が分裂し、
達成率やヒートマップがそれぞれの Mac の分しか映らない。同期は対象外。
主に作業する Mac 1台だけで使うのが素直。

## Xcode のない Mac に持ち込む

ビルド済みの `Koshimonban.app` をコピーすれば動く。ad-hoc 署名は特定のマシンに
紐づかないため、そのまま起動できる。
...
[SPEC.md](SPEC.md) に、何をなぜそう決めたかを書いてある。
挙動を変えたくなったときは、まずここを読んでほしい。
たいていの「素朴に良さそうな改善」は、1〜3 の優先順位と衝突する。

## 開発

### tests/

発動判定と URL 解析の検証。**Xcode 不要**、`swiftc` だけで走る。
...
最前面表示とフォーカス奪取が本当に成立するかを確かめた、使い捨ての検証アプリ。
...
デバッグモード中は記録されない。**確認が終わったら必ずオフに戻すこと。**
